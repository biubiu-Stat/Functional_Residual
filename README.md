...
- **`ffplot.envelop`** generates an $F_n$-$F_n$ plot with the "probable envelope" curves. The envelope is generated by the bootstrap method. It will yield the lower and upper critical values $L^*_{\alpha/2}(t)$ and $U^*_{1-\alpha/2}(t)$. Varying the value of $t$, the set $\mathcal{E}_{1-\alpha} = \{(L^*_{\alpha/2}(t), U^*_{1-\alpha/2}(t)), t \in (0,1)\}$ forms the envelope. Excursion of the 45-degree line outside $\mathcal{E}_{1-\alpha}$ may be regarded as evidence against the null that the model is correct.
